# quick_fix.py
"""
Script rápido para aplicar la corrección del error de duplicados
"""
...
print("Corrigiendo core/better_foot_shape.py...")
with open("core/better_foot_shape.py", "w", encoding="utf-8") as f:
    f.write(better_foot_code)
print("✓ Archivo corregido")

# Ahora generar las máscaras
print("\nGenerando máscaras PNG...")
try:
    from PIL import Image, ImageDraw, ImageFilter
    from core.better_foot_shape import foot_outline_anatomical
    ...
    for is_left in [True, False]:
        outline = foot_outline_anatomical(left=is_left, samples=300)
        
        outline_px = outline.copy()
        outline_px[:, 0] *= width
        outline_px[:, 1] *= height
        ...
        points = [(int(x), int(y)) for x, y in outline_px]
        draw.polygon(points, fill=255)
        
        # Suavizar bordes
        img = img.filter(ImageFilter.SMOOTH)
        
        filename = f"assets/foot_{'left' if is_left else 'right'}.png"
        img.save(filename)
        print(f"✓ Creado: {filename}")
    
    print("\n✅ TODO LISTO! Ahora ejecuta: python app.py")
    
except Exception as e:
    print(f"⚠ Error generando máscaras: {e}")
    print("Pero el archivo better_foot_shape.py está corregido.")
